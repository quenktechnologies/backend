...
import { attempt, doFuture } from '@quenk/noni/lib/control/monad/future';
    ...
    describe('updateMany()', function () {
        it('should update documents', () =>
            doFuture(function* () {
                ...
                let n = yield model.updateMany(
                    { customer: 'Sally' },
                    { customer: 'Hatty' }
                );

                let docs = yield dbkit.find(
                    'orders',
                    { customer: 'Hatty' },
                    { projection: { _id: 0 } }
                );
    ...
    describe('unsafeUpdate()', function () {
        it('should update documents', () =>
            doFuture(function* () {
                ...
                let n = yield model.unsafeUpdate(
                    { customer: 'Sally' },
                    { $set: { customer: 'Hatty' } }
                );
    ...
    describe('aggregate()', function () {
        it('should run a pipeline', () =>
            doFuture(function* () {
                ...
                let docs = yield model.aggregate([
                    { $match: { customer: 'Yui' } }
                ]);
