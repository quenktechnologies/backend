...
export class Render<A> extends Api<A> {
    constructor(public view: View, public status: number, public next: A) {
        super(next);
    }
 ...
 */
export const render = (view: View, status = 200): Action<void> =>
...
export const show = (path: Path, ctx: object = {}) =>
    doFuture<Content>(function* () {
        const eCons = interp(path, require);

        if (eCons.isLeft()) {
            return fromExcept(eCons);
        }

        const Cons = <Type>eCons.takeRight();

        const view: View = yield attempt(() => new Cons(ctx));
